...
export default function TherapistDirectoryPage() {
  ...
  const [savedTherapists, setSavedTherapists] = useState<string[]>(["1", "2"]);
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#F7F5FC] text-slate-800 antialiased font-sans">
      {/* ================= SIDEBAR NAVIGATION ================= */}
      <aside className="sticky top-0 flex h-screen w-64 shrink-0 flex-col justify-between border-r border-purple-100/60 bg-white/70 px-4 py-6 backdrop-blur-md">
        <div className="space-y-8">
          ...
          <div className="flex items-center gap-3 px-2">
            ...
            <span className="text-xl font-bold tracking-tight text-slate-900 font-serif">Adara</span>
          </div>
          ...
          <nav className="space-y-1">
            ...
            <SidebarItem href="/therapy" icon={<Heart className="h-4 w-4" />} label="Therapy" />
            <SidebarItem href="/wellness" icon={<Compass className="h-4 w-4" />} label="Wellness Hub" />
            <SidebarItem href="/directory" icon={<Users className="h-4 w-4" />} label="Therapist Directory" active />
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-purple-100 bg-white p-3 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="text-xs font-semibold text-slate-800">Aisha Khan</p>
                <p className="text-[11px] text-purple-600 font-medium cursor-pointer hover:underline">View Profile</p>
              </div>
            </div>
            <ChevronDown className="h-4 w-4 text-slate-400" />
          </div>

          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-100/60 via-purple-50/40 to-indigo-50/40 p-4 border border-purple-200/50 shadow-sm space-y-2">
            <p className="text-xs font-medium text-slate-700 leading-snug">You are not alone.</p>
            <p className="text-[11px] text-slate-500 leading-relaxed">Support is always available whenever you need it.</p>
            <div className="absolute -bottom-4 -right-4 h-16 w-16 opacity-30 bg-purple-300 rounded-full blur-xl pointer-events-none" />
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-y-auto">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-purple-100/60 bg-white/80 px-8 py-5 backdrop-blur-md">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Find qualified, compassionate professionals who can support your journey.</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="relative w-80">
              ...
              <input
                ...
                className="w-full rounded-full border border-purple-100 bg-[#F9F8FC] py-2 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:border-purple-400 focus:outline-none focus:ring-2 focus:ring-purple-200 transition"
              />
            </div>
            <button aria-label="Bookmarks" className="flex h-10 w-10 items-center justify-center rounded-full border border-purple-100 bg-white text-slate-600 shadow-sm transition hover:bg-purple-50">
              <Bookmark className="h-4 w-4" />
            </button>
            <button aria-label="Notifications" className="relative flex h-10 w-10 items-center justify-center rounded-full border border-purple-100 bg-white text-slate-600 shadow-sm transition hover:bg-purple-50">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center justify-between rounded-3xl border border-rose-100 bg-gradient-to-r from-rose-50/80 via-white to-rose-50/40 p-5 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-slate-900">Need immediate support?</h3>
                  <p className="text-xs text-slate-600 mt-0.5">If you are in crisis or feeling unsafe, help is available right now.</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
            ...
            {/* Filter Dropdowns & Active Tags */}
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-3">
                <FilterDropdown label="Specialisation" />
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                ...
                <span className="text-xs font-semibold text-purple-600 cursor-pointer hover:underline pl-2">Clear All</span>
              </div>
            </div>

            {/* Results Count & Sort */}
            <div className="flex items-center justify-between pt-2">
              <p className="text-xs font-semibold text-slate-500">32 therapists found</p>
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 rounded-xl border border-purple-100 bg-white p-1 shadow-sm">
                  <button aria-label="Grid view" className="rounded-lg bg-purple-100 p-1.5 text-purple-700"><Grid className="h-3.5 w-3.5" /></button>
                  <button aria-label="List view" className="rounded-lg p-1.5 text-slate-400 hover:text-slate-600"><List className="h-3.5 w-3.5" /></button>
                </div>
              </div>
            </div>
              ...
              {MOCK_THERAPISTS.map((therapist) => (
                ...
                >
                  <div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1">
                            <h4 className="font-semibold text-slate-900 text-xs">{therapist.name}</h4>
                          </div>
                          ...
                          <p className="text-[11px] text-slate-500 mt-0.5">{therapist.title}</p>
                          <p className="text-[11px] font-medium text-purple-600">{therapist.experience}</p>
                        </div>
                      </div>
                      <button
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-4">
                      {therapist.specialties.map((spec, idx) => (
                        <span key={idx} className="rounded-full bg-purple-50 px-2.5 py-1 text-[10px] font-medium text-purple-700">
                          {spec}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-2 mt-5 pt-3 border-t border-purple-50">
                    <button className="flex-1 rounded-xl border border-purple-200 bg-white py-2 text-[11px] font-semibold text-purple-700 transition hover:bg-purple-50">
                      View Profile
                    </button>
                    <button className="flex-1 rounded-xl bg-purple-600 py-2 text-[11px] font-semibold text-white shadow-sm transition hover:bg-purple-700">
                      Request Appointment
                    </button>
                  </div>
                </div>
            ...
            <div className="flex justify-center pt-4">
              <button className="flex items-center gap-2 rounded-full border border-purple-200 bg-white px-6 py-2.5 text-xs font-semibold text-purple-700 shadow-sm transition hover:bg-purple-50">
            ...
            <div className="flex items-center justify-between rounded-3xl bg-gradient-to-r from-slate-900 via-purple-950 to-slate-900 p-6 text-white shadow-lg mt-8">
              <div>
                <h3 className="text-base font-serif font-semibold">Need Help Now?</h3>
                <p className="text-xs text-purple-200 mt-0.5">We’re here for you. 24/7 confidential support and verified guidance.</p>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-purple-100 bg-white p-6 shadow-sm space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-900">Crisis Support</h3>
              </div>
              <div className="space-y-3">
                <CrisisContactItem
            ...
            <div className="rounded-3xl border border-purple-100 bg-white p-6 shadow-sm space-y-4">
              ...
              <div className="rounded-2xl border border-amber-100 bg-amber-50/40 p-4 space-y-2">
                ...
                <h4 className="text-xs font-semibold text-slate-800">Dr. Sarah Nakanjako</h4>
                <p className="text-[11px] text-slate-500">Requested on May 15, 2025</p>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-purple-100 bg-white p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-900">Saved Therapists</h3>
                <span className="text-xs font-medium text-purple-600 cursor-pointer hover:underline">View all</span>
              </div>
              <div className="flex items-center gap-3">
                <img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=100" className="h-10 w-10 rounded-full object-cover ring-2 ring-purple-100" alt="" />
                <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=100" className="h-10 w-10 rounded-full object-cover ring-2 ring-purple-100" alt="" />
                <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=100" className="h-10 w-10 rounded-full object-cover ring-2 ring-purple-100" alt="" />
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-purple-100 text-xs font-semibold text-purple-700 ring-2 ring-purple-50">
                  +2
                </div>
              </div>
            </div>
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-100/70 via-purple-50/50 to-indigo-50/50 p-6 border border-purple-200/60 shadow-sm space-y-3">
              ...
              <p className="text-[11px] font-medium text-purple-700">You&apos;ve already taken it.</p>
              <div className="absolute -bottom-6 -right-6 h-24 w-24 opacity-20 bg-purple-400 rounded-full blur-2xl pointer-events-none" />
            </div>
...
function SidebarItem({
  ...
  return (
    ...
    >
      <span className={active ? "text-purple-700" : "text-slate-400"}>{icon}</span>
      {label}
    </Link>
  );
}
...
function CrisisContactItem({ name, desc, number }: { name: string; desc: string; number: string }) {
  return (
    <div className="flex items-center justify-between rounded-2xl border border-purple-50 bg-[#F9F8FC] p-3 transition hover:border-purple-200">
      ...
      <button aria-label={`Call ${name}`} className="flex h-9 w-9 items-center justify-center rounded-full bg-rose-600 text-white shadow-md shadow-rose-100 transition hover:bg-rose-700">
        <PhoneCall className="h-4 w-4" />
      </button>
    </div>
  );
